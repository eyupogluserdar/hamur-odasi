...
(function () {
    window.App.Showcase = {
                    ...
                    <div style="margin-top:auto;">
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; margin-bottom:15px;">
                             <div style="background:rgba(255,255,255,0.05); padding:12px; border-radius:10px;">
                                <div style="font-size:0.75rem; color:#777; margin-bottom:4px; text-transform:uppercase; letter-spacing:0.5px;">Bekleme</div>
                                <div style="font-size:1.5rem; font-weight:bold; color:var(--color-primary); font-family:var(--font-heading);">
                                    ${ageHrs}s ${ageMins}d
                                </div>
                             </div>
                             <div style="background:rgba(255,255,255,0.05); padding:12px; border-radius:10px;">
        ...
        async openBatchModal(id) {
            ...
            // Re-bind delete
            modal.querySelector('.btn-delete-batch').onclick = () => {
            ...
            modal.querySelectorAll('.btn-action-modal').forEach(btn => {
                btn.onclick = async () => {
                    ...
                    if (action === 'complete') {
                        alert('Hamur kullanıldı ve stoktan düşüldü.');
                        // Re-render main view
                        this.render().then(html => { document.getElementById('main-view').innerHTML = html; this.afterRender(); });
                        return;
                    }
        ...
        renderBatchDetails(batch, recipe, allIngredients) {
            ...
            let tempType = 'room'; // for data-type

            if (batch.status === 'room') {
                phaseDuration = now - batch.startTime;
                statusText = "ODA (FERMANTASYON)"; // UPPERCASE confirmed
                phaseColor = '#f1c40f';
                ...
                statusText = "DOLAP (FERMANTASYON)"; // UPPERCASE with Dolap
                phaseColor = '#3498db';
                ...
                statusText = "ODA (FERMANTASYON)"; // Back to ODA
                phaseColor = '#e67e22'; // Keep orange to distinguish logic internally, but label is same
                mainActionBtn = `
            ...
            // We need to return the new HTML structure for the box
            // I'll reconstruct the return block to include the temperature input

            // --- INGREDIENT & TAG LOGIC REPEAT (simplified for context match or just use ... logic if I could, but need to output full string) ---
            let milkAmount = 0;
            let oilAmount = 0;
            let labelsHTML = '';
                ...
                <div style="background: rgba(255,255,255,0.05); padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                     ...
                     <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:10px; text-align:center;">
                        <div style="background:rgba(0,0,0,0.3); padding:10px; border-radius:8px;">
                            <div style="font-size:0.7rem; color:#aaa; margin-bottom:5px;">GÜNCEL ISI</div>
                        ...
                        <div style="background:rgba(0,0,0,0.3); padding:10px; border-radius:8px;">
                            <div style="font-size:0.7rem; color:#aaa; margin-bottom:5px;">BU EVRE</div>
                            <div style="font-size:1.4rem; font-weight:bold; color:${phaseColor}; overflow:hidden; white-space:nowrap;">${fmt(phaseDuration)}</div>
                        </div>
                        <div style="background:rgba(0,0,0,0.3); padding:10px; border-radius:8px;">
                            <div style="font-size:0.7rem; color:#aaa; margin-bottom:5px;">TOPLAM YAŞ</div>
                            <div style="font-size:1.4rem; font-weight:bold; overflow:hidden; white-space:nowrap;">${fmt(totalDuration)}</div>
                        </div>
                     </div>
                </div>
                ...
                <div style="font-size:0.95rem; color:#ccc; border-top:1px solid rgba(255,255,255,0.1); padding-top:15px;">
                    ...
                    <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                     ...
                     <div style="display:flex; justify-content:space-between; margin-bottom:8px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:8px;">
                        ...
                        <span style="font-family:monospace;">${batch.snapshot.flourAmount} g</span>
                    </div>
